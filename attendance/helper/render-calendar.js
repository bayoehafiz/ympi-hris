...
var renderCalendar = function(res, absence_data, attendance_data) {
    var calendar_data = [];
    if (res.length > 0) {
        // hide error block
        ...
        // Attaching any ABSENCE DAY if there any
        var day_scan = [];
        $.each(attendance_data, function(key, val) {
            var date = moment(val.scan_date, 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD');
            var time = moment(val.scan_date, 'YYYY-MM-DD HH:mm:ss').format('HH:mm');
            if (day_scan[date] == undefined) {
                day_scan[date] = [{ in: time, datescan: date }];
            } else {
                // prevent duplicate scan (in HOUR)
                var current_day = day_scan[date][0];
                if (moment(time, 'HH:mm').format('HH') !== moment(day_scan[date][0].in, 'HH:mm').format('HH')) {
                    if (current_day.out == undefined || moment(time, 'HH:mm').format('HH') !== moment(current_day.out, 'HH:mm').format('HH')) {
                        // check if it's actually IN or OUT
                        var time_diff = moment(time, 'HH:mm').diff(moment(current_day.in, 'HH:mm'), 'hours');
                        if (time_diff <= 12) {
                            day_scan[date].push({ out: time, datescan: date }); // <- it's the correct OUT scan
                        } else {
                            // check with the previous day scan
                            var previous_day = day_scan[moment(date, 'YYYY-MM-DD').subtract(1, 'days').format('YYYY-MM-DD')];
                            var temp = current_day.in;
                            current_day.in = time;
                            previous_day = [{ out: temp, datescan: date }];
                        }
                    }
                }
            }
        ...
        for (var key in day_scan) {
            $.each(day_scan[key], function(index, val) {
                if (val.in != undefined) {
                    $title = 'IN';
                    $color = '#1abc9c';
                    $time = val.in;
                ...
                calendar_data.push({
                    ...
                    title: $title,
                    description: $time,
                    start: new Date(moment(key, 'YYYY-MM-DD').format('YYYY/MM/DD')),
    ...
    $('#calendar').fullCalendar('removeEvents');
    $('#calendar').fullCalendar('addEventSource', calendar_data);
